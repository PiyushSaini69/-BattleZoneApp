    ...
    <View className="flex-1 bg-[#0B0F1A]">
      <View className="bg-slate-950 p-4 border-b border-slate-900 flex-row items-center justify-between">
        ...
        <Pressable 
          ...
          className="bg-[#7C3AED] w-8 h-8 rounded-lg items-center justify-center border"
          style={{
            borderColor: '#7C3AED',
            shadowColor: '#7C3AED',
                ...
                {['payment', 'tournament', 'technical', 'other'].map((cat) => (
                  <Pressable
                    ...
                    className={`px-3 py-1.5 rounded-lg border mb-2 w-[48%] items-center ${
                      category === cat ? 'border-purple-500' : 'bg-slate-900 border-slate-850'
                    }`}
                    style={category === cat ? { backgroundColor: 'rgba(124, 58, 237, 0.2)' } : null}
                  >
