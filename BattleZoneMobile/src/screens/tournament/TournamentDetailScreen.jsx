...
export default function TournamentDetailScreen({ route, navigation }) {
  ...
  if (!tournament) {
    return (
      <View className="flex-1 bg-[#0B0F1A] justify-center items-center">
        <Text className="text-slate-450 text-sm font-semibold">Loading tournament details...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      <View className="bg-slate-900 border-b border-slate-950 p-6 items-center">
        ...
        <Text className="text-white text-xl font-black text-center mb-4">{tournament.title}</Text>
      ...
      <View className="p-4">
        <GlassCard className="p-5 flex-row flex-wrap justify-between mb-6">
          <View className="w-[48%] mb-4 flex-row items-center">
            ...
            <View>
              <Text className="text-slate-450 text-[8px] uppercase font-bold mt-0.5">Scheduled</Text>
          ...
          <View className="w-[48%] mb-4 flex-row items-center">
            ...
            <View>
              <Text className="text-slate-450 text-[8px] uppercase font-bold mt-0.5">Game Mode</Text>
          ...
          <View className="w-[48%] flex-row items-center">
            ...
            <View>
              <Text className="text-slate-450 text-[8px] uppercase font-bold mt-0.5">Prize Pool</Text>
          ...
          <View className="w-[48%] flex-row items-center">
            ...
            <View>
              <Text className="text-slate-450 text-[8px] uppercase font-bold mt-0.5">Entry Fee</Text>
        ...
        {isRegistered ? (
          <GlassCard className="p-4 bg-purple-950/15 border-purple-500/30 items-center mb-6">
            ...
            <Text className="text-slate-450 text-[10px] text-center mb-4 leading-relaxed">
            ...
            {isLive ? (
              <Button 
                ...
                className="w-full bg-emerald-650 border border-emerald-500 shadow-md"
              />
            ) : (
              <Button 
        ...
        <GlassCard className="p-4 mb-6">
          <View className="flex-row justify-between mb-2">
            ...
            <Text className="text-[#C084FC] font-extrabold text-xs">
              {tournament.filledSlots}/{tournament.totalSlots} Slots
            </Text>
          </View>
          <View className="h-1.5 bg-slate-900 border border-slate-850 rounded-full overflow-hidden">
            <View className="h-full bg-[#7C3AED] rounded-full" style={{ width: `${progress}%` }} />
          </View>
        </GlassCard>

        <GlassCard className="p-5">
          <Text className="text-white font-extrabold text-xs uppercase tracking-wider mb-4">Players Joined ({participants.length})</Text>
          {participants.length === 0 ? (
            <Text className="text-slate-455 text-xs font-semibold text-center py-4">Be the first warrior to enter the arena!</Text>
          ) : (
            participants.map((player) => (
              <View key={player._id} className="flex-row justify-between items-center py-3 border-b border-slate-850/60">
                <Text className="text-white font-bold text-xs">{player.userId?.username || 'Gamer'}</Text>
                <Text className="text-slate-450 text-[9px] font-bold uppercase">Slot #{player.slotNumber}</Text>
              </View>
            ))
          )}
        </GlassCard>
      ...
      <Modal transparent visible={showRegisterModal} animationType="slide">
        <View className="flex-1 bg-black/75 justify-center items-center p-6">
          <GlassCard className="w-full max-w-sm p-6 bg-slate-950 border border-purple-500/35">
            <Text className="text-[#C084FC] text-base font-black text-center mb-2 uppercase tracking-wide">Join Arena</Text>
            ...
            <View className="bg-slate-900 rounded-xl p-3.5 mb-6 flex-row justify-between items-center">
              <Text className="text-slate-450 text-[10px] font-bold uppercase">Entry Cost</Text>
            ...
            <View className="flex-row justify-between">
              ...
              >
                <Text className="text-slate-300 text-center font-bold text-xs uppercase">Cancel</Text>
              </Pressable>
              <Pressable 
