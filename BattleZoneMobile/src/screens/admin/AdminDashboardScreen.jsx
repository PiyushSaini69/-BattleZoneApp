    ...
    <View className="flex-1 bg-[#0B0F1A]">
      <View className="bg-slate-950 p-4 border-b border-slate-900 flex-row items-center justify-between">
        ...
        <Pressable 
          ...
          className="bg-red-600 w-8 h-8 rounded-lg items-center justify-center border border-red-500 shadow-md shadow-red-500/20"
        >
      ...
      >
        ...
        <View className="flex-row justify-between mb-6">
          <GlassCard className="w-[31%] p-3 items-center">
            ...
            <Text className="text-slate-450 text-[8px] uppercase font-bold text-center mt-0.5">Users</Text>
          </GlassCard>
        ...
        {withdrawals.filter(w => w.status === 'pending').length === 0 ? (
          ...
          withdrawals.filter(w => w.status === 'pending').map((w) => (
            <GlassCard key={w._id} className="mb-4 p-4 bg-slate-900/40 border-white/5 flex-row justify-between items-center">
